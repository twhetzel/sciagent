...
| Breast cancer proteomics | disease only; assay inferred from query | Disease-only facet ladder when tissue is absorbed by the disease phrase (“breast cancer”); proteomics inference |
...
| Breast cancer proteomics | `disease=breast cancer`, `assay` inferred | MONDO:0007254 (breast cancer) |

Notes:

- **Organism** is not inferred from “public”; `TAXONOMY:"9606"` is added only when the user names an organism (e.g. `human`).
- **Proteomics / metabolomics / RNA-seq** in the query text are mapped to OmicsDI `omics_type` filters when the assay facet slot is empty (`tools/omicsdi_dataset_search.py` → `_infer_assay_from_query`).
...
- **Breast cancer breast tissue** resolves disease only today because “breast” is consumed by the disease phrase; OmicsDI still applies the inferred proteomics filter.
...
| Breast cancer proteomics | `disease:"Breast cancer" AND omics_type:"Proteomics"` | same as strict (no tissue facet) | same as strict | `disease:"Breast cancer"` |
...
```bash
cd server
uv run pytest tests/test_breast_cancer_omicsdi_query.py tests/test_omicsdi_evidence_extraction.py \
  tests/test_omicsdi_dataset_search.py tests/test_omicsdi_vocab.py -q
```
